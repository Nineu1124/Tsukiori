...
const state = {
  ...
  filePreviewContent: '', nativeCapabilities: null, turnStartedAt: 0, lastPrompt: '',
};
...
function appendEvent(target,event,scroll=true) {
  ...
  else if (event.type === 'turn.completed') { const divider=document.createElement('div');divider.className='turn-divider';divider.textContent=event.payload.status==='failed'?'Turn failed':'Turn completed';target.append(divider);state.assistantDraft=null;setTurnActivity(event.payload.status==='failed'?'error':'complete',event.payload.status==='failed'?'Turn 执行失败':'回复完成'); }
  else if (event.type === 'runtime.error') { target.append(messageNode('error','Runtime error',event.payload.message)); state.assistantDraft=null;setTurnActivity('error',event.payload.message||'Runtime 执行失败'); }
...
function acceptEvent(event) { state.recentEvents.push(event); if (state.recentEvents.length>300) state.recentEvents.splice(0,state.recentEvents.length-300); appendTerminalEvent(event); if(event.type==='turn.started')setTurnActivity('running','模型正在思考并执行任务…');if(event.type==='assistant.delta')setTurnActivity('running','正在接收回复…');if (event.sessionId===state.activeSessionId&&!event.type.startsWith('terminal.')) appendEvent(byId('conversation'),event); if (event.type==='permission.requested'||event.type==='permission.resolved') void reloadSnapshot(); if (event.type==='turn.started'||event.type==='turn.completed'||event.type==='runtime.error'||event.type==='team.started'||event.type==='team.completed') void reloadSnapshot(); }
...
function renderAgentSettings(){const summary=byId('agent-settings-summary'),list=byId('agent-settings-list');if(!summary||!list)return;clear(summary);clear(list);summary.append(diagnosticTile('Runtime',state.runtimes.filter((item)=>item.available).length),diagnosticTile('Agent Team',state.teams.length),diagnosticTile('运行中',state.teams.filter((item)=>item.status==='running').length));for(const runtime of state.runtimes)list.append(capabilityItem(runtime.name,(runtime.capabilities??[]).join(' · ')||runtime.error||'未声明能力',runtime.available?runtime.supportLevel:'不可用'));for(const team of state.teams)list.append(capabilityItem(team.name,`${team.memberSessionIds.length} Agent · ${new Date(team.updatedAt).toLocaleString()}`,team.status));}
function renderMemorySettings(){const root=byId('memory-settings-list');if(!root)return;clear(root);for(const project of state.projects){const sessions=state.sessions.filter((item)=>item.projectId===project.id);root.append(capabilityItem(project.name,`${sessions.length} Session · ${sessions.reduce((sum,item)=>sum+(item.turnCount||0),0)} Turn`,state.settings.persistConversation?'Transcript 已启用':'仅内存'));}if(!root.children.length)root.append(emptyText('添加项目后显示本地记忆绑定'));}
...
function renderSettingsData(){const s=state.settings??{};byId('setting-language').value=s.language??'zh-CN';byId('setting-auto-update').checked=s.autoUpdate!==false;byId('setting-start-minimized').checked=s.startMinimized===true;byId('setting-persist-conversation').checked=s.persistConversation!==false;byId('setting-confirm-high-risk').checked=s.confirmHighRisk!==false;byId('setting-theme').value=s.theme??'light';byId('setting-density').value=s.density??'comfortable';byId('setting-reduce-motion').checked=s.reduceMotion===true;byId('setting-terminal-shell').value=s.terminalShell??'powershell';fillRuntime(byId('setting-default-runtime'),s.defaultRuntime??'codex');fillProvider(byId('setting-default-provider'),byId('setting-default-runtime').value,s.defaultProviderId);renderProviderList();renderSettingsRuntimes();renderSettingsProjects();renderUsage();renderAgentSettings();renderMemorySettings();renderTraceSettings();renderNativeSettings();setText('about-version','Desktop '+(state.versions.desktop??'—')+' · Protocol '+(state.versions.protocol??'—'));}
...
function bindUi(){
  ...
  byId('retry-turn').addEventListener('click',()=>{const latest=[...state.recentEvents].reverse().find((event)=>event.sessionId===state.activeSessionId&&event.type==='user.message');byId('prompt-input').value=state.lastPrompt||String(latest?.payload?.text??'');byId('prompt-form').requestSubmit();});
  ...
  byId('settings-restart-terminal').addEventListener('click',async()=>{await saveSettings();if(activeSession())await restartTerminal();setText('terminal-settings-status','已保存并重启当前终端');});byId('refresh-mcp').addEventListener('click',refreshNativeSettings);byId('refresh-skills').addEventListener('click',refreshNativeSettings);byId('settings-create-team').addEventListener('click',openTeamDialog);byId('refresh-memory').addEventListener('click',renderMemorySettings);byId('refresh-trace').addEventListener('click',renderTraceSettings);byId('run-doctor').addEventListener('click',runDoctor);byId('export-diagnostic-settings').addEventListener('click',exportDiagnosticSettings);
