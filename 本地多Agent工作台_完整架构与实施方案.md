...
- [ ] 关键技术前提已通过，可以进入宿主骨架开发
- 前置依赖：T0.1、T0.2、T0.3、T0.4
- 交付物：阶段 0 Gate 报告
- Checkpoints：
  - [ ] T0.1 至 T0.4 全部完成
  - [ ] 两种 Runtime 均证明控制命令与事件流可以分离
  - [ ] 权限能力矩阵覆盖可拦截、仅观察、不可见和未知状态
  - [ ] GUI、Daemon、Runtime 崩溃验证没有未解释的数据破坏
  - [ ] ADR、Schema 和 Fixture 已进入版本控制并可在 CI 中校验
...
- Checkpoints：
  - [ ] Runtime、PTY、事件、Blob、Diff 和日志均有大小或资源上限
  - [ ] 所有持久化 Sink 之前执行脱敏、截断和不可信内容处理
  - [ ] 诊断包默认不包含源码、完整 Prompt、Raw Payload 或凭据
  - [ ] 用户明确勾选敏感内容时再次脱敏并显示预计文件大小
  - [ ] 三个并行 Session 的启动、流式事件和 Diff 性能达到已记录基线
...
- Checkpoints：
  - [ ] Windows x64 可完成安装、升级、卸载和重新安装
  - [ ] Desktop 与 Daemon 有独立版本并在更新前检查协议兼容性
  - [ ] 数据库 Migration 前自动备份，失败时保留可恢复副本
  - [ ] 安装包和更新包通过签名与来源验证
  - [ ] 回滚应用版本时能识别并阻止不兼容数据库降级
